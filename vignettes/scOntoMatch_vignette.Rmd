...
## Match ontology
### First trim the ontology tree per dataset
...
adatas = ontoMatch(adata1 = ad_minimal, adata2 = ad_two_minimal, anno_col = anno_col_minimal, onto_id_col = onto_id_col, ont = ont)
...
plotOntoTree(ont = ont, 
                          onts = names(getOntologyId(adatas[[1]]$obs[['cell_ontology_base']], ont = ont)), 
                          ont_query = names(getOntologyId(adatas[[1]]$obs[['cell_ontology_base']], ont = ont)),
                          plot_ancestors = TRUE,  roots = 'CL:0000548',
                          fontsize=25)
```

### Plot a unified ontology tree using both datasets after matching
```{r}
